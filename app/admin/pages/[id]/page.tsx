    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-bold text-white font-serif">Edit Page</h2>
            <p className="text-sm text-slate-400">Modify properties or restructure contents of: {title}</p>
          </div>
        </div>
        <button
          type="submit"
      ...
      <div className="flex border-b border-slate-800">
        ...
        >
          {pageType === "HOME" ? "2. Homepage Layout" : `2. Content Blocks (${sections.length})`}
        </button>
      </div>
          ...
          <div className="space-y-6">
            ...
            <div className="rounded-2xl border border-slate-800 bg-slate-900/20 p-6 space-y-4">
              ...
              <p className="text-xs text-slate-400 leading-relaxed">
                Updating a page is immediate on the website frontend. However, it's recommended to preview changes in draft mode if your public clients are visiting your dynamic paths concurrently.
              </p>
            </div>
          </div>
        </div>
      ...
      {activeTab === "sections" && pageType !== "HOME" && (
      ...
      {activeTab === "sections" && pageType === "HOME" && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="bg-slate-900/50 border border-slate-800/80 rounded-2xl p-6">
            ...
            <div className="space-y-4">
              <div>
                ...
                <textarea 
                  value={(sections[0] as any)?.content?.heroTitle || (sections[0] as any)?.heroTitle || ""} 
                  onChange={(e) => {
                    const up: any = [...sections];
                    if (!up[0]) up[0] = { id: "home-data", type: "hero", content: {} };
                    up[0].heroTitle = e.target.value;
                    setSections(up);
                  }}
              ...
              <div>
                ...
                <input 
                  type="text" 
                  value={(sections[0] as any)?.content?.heroSubtitle || (sections[0] as any)?.heroSubtitle || ""} 
                  onChange={(e) => {
                    const up: any = [...sections];
                    if (!up[0]) up[0] = { id: "home-data", type: "hero", content: {} };
                    up[0].heroSubtitle = e.target.value;
                    setSections(up);
                  }}
                  className="w-full bg-slate-950 border border-slate-800 rounded-lg p-3 text-sm text-white" 
                />
              ...
              <div>
                ...
                <input 
                  type="text" 
                  value={(sections[0] as any)?.content?.heroBtn || (sections[0] as any)?.heroBtn || ""} 
                  onChange={(e) => {
                    const up: any = [...sections];
                    if (!up[0]) up[0] = { id: "home-data", type: "hero", content: {} };
                    up[0].heroBtn = e.target.value;
                    setSections(up);
                  }}
                  className="w-full bg-slate-950 border border-slate-800 rounded-lg p-3 text-sm text-white" 
                />
              ...
              <div>
                <label className="block text-xs font-medium text-slate-400 mb-1">Background Image URL</label>
                <input 
                  type="text" 
                  value={(sections[0] as any)?.content?.heroImg || (sections[0] as any)?.heroImg || ""} 
                  onChange={(e) => {
                    const up: any = [...sections];
                    if (!up[0]) up[0] = { id: "home-data", type: "hero", content: {} };
                    up[0].heroImg = e.target.value;
                    setSections(up);
                  }}
                  className="w-full bg-slate-950 border border-slate-800 rounded-lg p-3 text-sm text-white" 
                />
          ...
          {/* BRAND INTRO SECTION */}
          <div className="bg-slate-900/50 border border-slate-800/80 rounded-2xl p-6">
            <h3 className="text-lg font-semibold text-white mb-4">Brand Intro Section</h3>
            <div className="space-y-4">
              <div>
                <label className="block text-xs font-medium text-slate-400 mb-1">Title</label>
                <input 
                  type="text" 
                  value={(sections[0] as any)?.content?.brandTitle || (sections[0] as any)?.brandTitle || ""} 
                  onChange={(e) => {
                    const up: any = [...sections];
                    if (!up[0]) up[0] = { id: "home-data", type: "hero", content: {} };
                    up[0].brandTitle = e.target.value;
                    setSections(up);
                  }}
                  className="w-full bg-slate-950 border border-slate-800 rounded-lg p-3 text-sm text-white" 
                />
              ...
              <div>
                <label className="block text-xs font-medium text-slate-400 mb-1">Paragraph</label>
                <textarea 
                  value={(sections[0] as any)?.content?.brandSubtitle || (sections[0] as any)?.brandSubtitle || ""} 
                  onChange={(e) => {
                    const up: any = [...sections];
                    if (!up[0]) up[0] = { id: "home-data", type: "hero", content: {} };
                    up[0].brandSubtitle = e.target.value;
                    setSections(up);
                  }}
                  className="w-full bg-slate-950 border border-slate-800 rounded-lg p-3 text-sm text-white" rows={3} 
                />
              </div>
              <div>
                <label className="block text-xs font-medium text-slate-400 mb-1">Button Text</label>
                <input 
                  type="text" 
                  value={(sections[0] as any)?.content?.brandBtn || (sections[0] as any)?.brandBtn || ""} 
                  onChange={(e) => {
                    const up: any = [...sections];
                    if (!up[0]) up[0] = { id: "home-data", type: "hero", content: {} };
                    up[0].brandBtn = e.target.value;
                    setSections(up);
                  }}
                  className="w-full bg-slate-950 border border-slate-800 rounded-lg p-3 text-sm text-white" 
                />
              ...
              <div>
                ...
                <input 
                  type="text" 
                  value={(sections[0] as any)?.content?.brandImg || (sections[0] as any)?.brandImg || ""} 
                  onChange={(e) => {
                    const up: any = [...sections];
                    if (!up[0]) up[0] = { id: "home-data", type: "hero", content: {} };
                    up[0].brandImg = e.target.value;
                    setSections(up);
                  }}
                  className="w-full bg-slate-950 border border-slate-800 rounded-lg p-3 text-sm text-white" 
                />
